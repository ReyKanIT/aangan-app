...
import type { AanganEvent, User } from '@/types/database';
...
/**
 * EventInviteCard — The share/CTA block on an event page.
 *
 * Why family-graph context matters: WhatsApp previews the first line, so we
 * lead with "[Inviter] ने आपको न्यौता भेजा है" — that single line is what
 * turns a forward into a qualified invite instead of a generic link drop.
 */
export default function EventInviteCard({ event, inviter, className = '' }: Props) {
  ...
  const inviterName = inviter?.display_name_hindi || inviter?.display_name;

  const inviteMessage = [
    inviterName ? `${inviterName} ने आपको न्यौता भेजा है 💌` : `आपको न्यौता है 💌`,
    '',
    `${typeInfo.emoji} ${eventTitle}`,
  ...
  const mailtoHref = `mailto:?subject=${encodeURIComponent(`न्यौता — ${eventTitle}`)}&body=${encodeURIComponent(inviteMessage)}`;
  ...
  const handleCopyLink = async () => {
    try {
      ...
      window.prompt('लिंक कॉपी करें — Copy this link:', url);
    }
  };
  ...
  const handleCopyMessage = async () => {
    try {
      ...
      window.prompt('संदेश कॉपी करें — Copy message:', inviteMessage);
    }
  };
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: eventTitle, text: inviteMessage, url });
      } catch { /* user cancelled */ }
    }
  };

  const handleCalendar = () => downloadEventIcs(event);
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-2">
          ...
          >
            ...
            <div>
              <p className="font-semibold">कैलेंडर में जोड़ें</p>
              <p className="text-brown-light">Add to Calendar (.ics)</p>
            </div>
          </button>
          ...
          >
            <span className="text-xl shrink-0">📋</span>
            <div>
              <p className="font-semibold">पूरा न्यौता कॉपी करें</p>
