...
- **No silent feature removal** — `CRITICAL_FEATURES.md` at the repo root lists UI features that must always be present (Google sign-in, sign-out confirm dialog, tree-card buttons, etc.). Before removing ANY UI element from any file, check that file against the manifest. If the element is listed, STOP and ask Kumar in chat — do not remove without explicit OK. The pre-build hook (`npm run check:critical`) enforces this automatically; if it fails after your edit, you broke a critical feature. Never bypass the check by deleting the manifest entry — only delete it after Kumar explicitly authorizes the removal in the same conversation. Background: this rule was added 2026-05-03 after the v0.10.1 commit silently stripped Google sign-in for ~7 days.
- **Hindi text in JSX MUST be wrapped in `{'...'}` JS strings, not bare JSX text** — JSX text content (`<button>हिंदी</button>`) and HTML attribute values (`aria-label="हिंदी"`) do NOT process `\u` escape sequences. Some tooling silently saves Devanagari chars as literal `से...` strings, which then render as raw escape text on the live site instead of Hindi. The fix is always: wrap in a JS expression — `{'हिंदी'}` for text, `aria-label={'हिंदी'}` for attrs. The pre-build check catches the Google button case explicitly; for new Hindi strings, follow the convention of `{'अप...'}` you'll see throughout the codebase. Background: this rule was added 2026-05-03 after v0.13.16 shipped with the Google button label rendering as literal `Google से जारी ...` text.
